...
def get_actions_blob(video_name_1, video_name_2, au_action, posiface, smile, parallel_run_settings):
    ...
    if os.path.exists(str(pathlib.Path(os.path.join(parallel_run_settings['csv_path'], video_name_1 + '_' + video_name_2, 'Stage_4', 'Actions', 'narrative_coarse.csv')))):
        return print('Stage 4 Actions File Exists')

    start = datetime.now()
    posiface_count_blob = sap.get_all_blob(video_name_1, video_name_2, parallel_run_settings)
    smiling_count_blob = sas.get_all_blob(video_name_1, video_name_2, parallel_run_settings)
    au_blob = saa.get_all_blob(video_name_1, video_name_2, parallel_run_settings)
    ...
    au_blob['blob'] = au_blob.blob.apply(lambda x: x if au_action == 1 else '')
    ...
    print('Stage 4 Actions Time: ', datetime.now() - start)
