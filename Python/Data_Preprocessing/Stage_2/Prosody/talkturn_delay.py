...
def extract_delay(video_1, video_2, parallel_run_settings):
    '''
    Computes for the talkturn delay
    :return: none
    '''
    # Mark - add a condition that stops the function from running again if file exists
    if os.path.exists(str(pathlib.Path(os.path.join(parallel_run_settings['csv_path'], video_1 + '_' + video_2, 'Stage_2', 'talkturn_delay.csv')))):
...
if __name__ == '__main__':
    extract_delay(video_1='zoom_F', video_2='zoom_M')
